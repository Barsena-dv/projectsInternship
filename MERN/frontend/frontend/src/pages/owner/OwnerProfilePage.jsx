...
const OwnerProfilePage = () => {
  ...
  const T = { title: { fontSize: '0.9rem', fontWeight: 700, color: '#f8fafc', margin: 0 }, meta: { fontSize: '0.78rem', color: '#94a3b8', margin: '2px 0 0' } };
    ...
    <div className="owner-page-enter">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: '1.25rem', alignItems: 'start' }}>
        ...
        <div className="owner-section-card" style={{ textAlign: 'center', padding: '1.5rem' }}>
          <div style={{ width: '72px', height: '72px', borderRadius: '18px', background: 'rgba(99,102,241,0.15)', border: '1.5px solid rgba(129,140,248,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#a5b4fc', fontWeight: 800, fontSize: '1.5rem', margin: '0 auto 0.875rem' }}>
            {initials(user?.full_name)}
          </div>
          <p style={T.title}>{user?.full_name || 'User'}</p>
          <p style={T.meta}>{user?.email}</p>
          <span style={{ display: 'inline-block', marginTop: '0.625rem', padding: '0.2rem 0.7rem', borderRadius: '5px', background: 'rgba(99,102,241,0.15)', color: '#a5b4fc', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', border: '1px solid rgba(129,140,248,0.3)' }}>
          ...
          <div style={{ marginTop: '1.25rem', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '1rem' }}>
            {[['Member since', user?.createdAt ? new Date(user.createdAt).getFullYear() : '—'],['Status', '● Active']].map(([k, v]) => (
              <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', padding: '0.3rem 0' }}>
                ...
                <span style={{ fontWeight: 600, color: k === 'Status' ? '#10b981' : '#f8fafc' }}>{v}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.2rem', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', padding: '0.2rem', width: 'fit-content', boxShadow: 'none' }}>
            {TABS.map((tab) => (
              <button key={tab} type="button" onClick={() => setActiveTab(tab)}
                style={{ padding: '0.4rem 1rem', borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '0.82rem', fontWeight: 600, fontFamily: 'inherit', transition: 'all 0.15s', background: activeTab === tab ? '#6366f1' : 'transparent', color: activeTab === tab ? '#fff' : '#94a3b8', boxShadow: activeTab === tab ? '0 4px 12px rgba(99,102,241,0.3)' : 'none' }}
              >{tab}</button>
            ))}
          </div>
            ...
            <div className="owner-section-card">
              <div className="owner-section-header">
                ...
                <button type="button" onClick={() => setEditing((p) => !p)}
                  style={{ fontSize: '0.78rem', fontWeight: 600, padding: '0.3rem 0.75rem', borderRadius: '7px', border: '1px solid rgba(255,255,255,0.1)', background: editing ? 'rgba(239,68,68,0.15)' : 'rgba(0,0,0,0.2)', color: editing ? '#fca5a5' : '#cbd5e1', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s' }}
              ...
              {editing ? (
                ...
                INFO.map(([k, v]) => (
                  <div key={k} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.625rem 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <span style={{ fontSize: '0.825rem', color: '#94a3b8', fontWeight: 500 }}>{k}</span>
                    <span style={{ fontSize: '0.875rem', color: '#f8fafc', fontWeight: 600 }}>{v || '—'}</span>
                  </div>
                ))
              )}
            </div>
            ...
            <div className="owner-section-card">
              ...
              {[
                ...
                <div key={item.title} style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '10px', padding: '1rem', marginBottom: '0.75rem' }}>
                  <p style={{ margin: 0, fontWeight: 700, fontSize: '0.875rem', color: '#f8fafc' }}>{item.title}</p>
                  <p style={{ margin: '0.25rem 0 0.75rem', fontSize: '0.78rem', color: '#94a3b8' }}>{item.desc}</p>
                  <button type="button" disabled={item.disabled}
                    style={{ padding: '0.4rem 1rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: item.disabled ? 'rgba(0,0,0,0.1)' : 'rgba(99,102,241,0.15)', color: item.disabled ? '#64748b' : '#a5b4fc', fontSize: '0.8rem', fontWeight: 600, cursor: item.disabled ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}
                  >{item.action}</button>
                </div>
              ))}
            </div>
          ...
          {activeTab === 'Activity' && (
            <div className="owner-section-card" style={{ textAlign: 'center', padding: '3rem' }}>
              <p style={{ color: '#94a3b8', fontSize: '0.875rem', margin: 0 }}>Activity history coming soon.</p>
            </div>
          )}
        </div>
      </div>
